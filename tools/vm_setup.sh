...
PYEOF
  ...
  # PAYMENT-ENDGAME INTERLOCK — the one precondition whose breach is not
  # automatically recoverable, and the only one that rested entirely on a
  # human remembering FLIP_RUNBOOK 6c. If the driver arms with
  # CONT_DAILY_REPORTS=True, the 14:00 IST send pulls the ENTIRE unstamped
  # rebuild cohort into one sheet, sends it as a payment document and
  # stamps it — after which recal_regen_sheets' stray-stamp gate returns 2
  # forever and the sheets of record can no longer be regenerated by the
  # tool. Assert it in the machine, not the runbook (r-loop 4).
  # Skipped once the regen has completed, which is when True is correct.
  if [ ! -f "$HOME/hl-pipeline/reports/2026-08-15/.regen-v2-done" ] || \
     [ ! -f "$HOME/hl-pipeline/reports/2026-08-16/.regen-v2-done" ]; then
    "$HOME/.local/bin/uv" run python -c \
      'import sys; sys.path.insert(0,"."); from pipeline import config as C; sys.exit(1 if C.CONT_DAILY_REPORTS else 0)' \
      || { echo "FATAL: CONT_DAILY_REPORTS is True but the 08-15/08-16 regen" >&2
           echo "  has not completed. Arming now would let the 14:00 IST send" >&2
           echo "  stamp the unstamped rebuild cohort and permanently block" >&2
           echo "  recal_regen_sheets. Set it False, commit, redeploy" >&2
           echo "  (FLIP_RUNBOOK step 6c), then re-run." >&2
           exit 1; }
    echo "interlock OK: CONT_DAILY_REPORTS=False (regen not yet complete)"
  fi
