// r-loop-11 review workflow (RULED by Adnaan 2026-08-19: runs regardless of
// iteration 10's quiet judgment, as a confirmation pass). Edited from the
// committed iter10 snapshot: regressions lane retargeted at the ITERATION-10
// FIX COMMIT 6dd2e64 (16 fixes, the executor's own — treat as prime target),
// accepted-behaviours 29-33 appended for the r10 rulings (mechanism defects
// stay reportable), suite refreshed (641/floor 619). LOOP_START, 2-vote
// refute discipline and the tests-coverage lane kept.
export const meta = {
  name: 'flip-review-iter11',
  description: 'r-loop 11: adversarial review of the continuous-pipeline flip work, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-10 fixes' },
...
30. r-loop 10: a PERMANENTLY-refusing daily resume (unreadable record,
   missing CSV, deleted counted row) writes reports/<day>/.wedged + one
   Telegram alert, and the day-agnostic scan skips wedged days LOUDLY so
   later days proceed; transient failures never wedge and retry next
   tick; the daily-report CLI takes the run lock. A wedge fired for a
...
33. r-loop 10 (ruling C hardening): ORPHANED paid-piece memory (a
   recorded id absent from the current tree) voids the id-keyed match —
   every not-in-memory DELIVERED node of that tree is excluded LOUDLY
   with no stamp and the root keeps re-entering until a human
   reconciles; money-safe (withheld hours are hand-recoverable,
   double-pays are not) is the RULED direction. A false-orphan lockout
   of genuinely-new hours with no loud line, or a silent double-pay
   that still slips through, IS a finding.
`

const LANES = [
  { key: 'regressions-r10', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-10 fix commit 6dd2e64 (16 fixes, all written by the same executor whose work you are checking — Adnaan ordered this pass BECAUSE the loop's worst findings have always been regressions from the previous iteration's fixes; do not extend it any benefit of the doubt). Read \`git show 6dd2e64\` IN FULL, then attack each change. The 16 changes: (1) apply_fixes pre-cut fixlog persist with a \`persisted\` watermark — attack: multi-cut plans, the watermark vs the failed-step tail, the empty-applied skip (the old code appended an empty record; the new code does not), double-propagation through fixlog-walk + tail, and the OSError path from the pre-cut append itself; (2) the .wedged mechanism + daily-report CLI run lock — attack: a transient condition that can reach a wedge path, a wedged TODAY blocking its own fresh generation, .wedged interactions with doc_sent/settled days, lock leak on exception, and whether the wedge alert can itself wedge; (3) the evidence-scoped worker-death count (md5-embedded marker + last-successful-return anchor) — attack: same-second ts ties (ledger _now() is second-precision, the query uses ts >), old-format markers from before this commit, the anchor state set (is QUARANTINED->healed re-entry covered? REVALIDATING?), and md5 changing between death 1 and death 2; (4) the zip-heal deferral — attack: the prev_md5= rsplit parse (details containing prev_md5 elsewhere), heals of rows whose md5 was ALREADY empty, the clear-then-stamp kill window, interplay with the by_md5 dedupe that runs in the same download-completion block, and whether the deferred clear can fire on a row whose stamps were legitimately re-earned in between; (5) _adopted_segments newest-event — attack: same-ts events ordering; (6) fix_key_hygiene button canonicalization + unbound-key strip and fix_sentinels _FLOAT_RE rewrite — attack: VALID tokens eaten (arrow-key display names in the buttons column? keys legitimately unbound but bound-in-context games?), the no-keybind guard, precision loss from .1f on nonconformant-but-meaningful cells, fix_v1_to_v2's variant, and whether stripping unbound keys breaks the fan-out/coverage checks or key_binding.json coverage; (7) the four motion_track guards — attack: the broad \`except Exception\` swallowing programming errors (numpy shape bugs) as 'not decodable', and the translate-path mdx=None restructure (is the else-branch warning correct? does sync_report stay coherent?); (8) the orphan-memory exclusion — attack: false orphans (a paid piece deleted by rebuild-reset while the tree lives on, a REJECTED-then-deleted child), the per-sheet noise loop, _tree_has_uncounted_accepted's orphan walk cost/correctness, and whether an orphaned tree's REJECT LABELS still flow (they should — only DELIVERED hours are excluded). Also attack the 18 new/changed tests in the same commit for vacuous assertions. Report at most 5 findings — the ones you can PROVE.` },
  { key: 'payment-split', prompt:
    `Attack the NEW two-mark payment logic in pipeline/reports.py (build_sheet_rows, mark_accepted_reported, write_payment_sheet) plus its callers in pipeline/run.py and tools/recal_regen_sheets.py, tools/recal_refix_reset.py, tools/recal_rebuild_reset.py, pipeline/ledger.py (supersede) and pipeline/ingest.py (quarantine heal). Prove or disprove, by RUNNING code against a scratch sqlite ledger in /tmp: (1) uploaded hours are still counted exactly once across all sheets; (2) every DELIVERED node's hours reach exactly one sheet, and none is lost; (3) reject labels appear exactly once; (4) no root can re-enter forever; (5) a kill between the sheet send and either stamp cannot double-pay; (6) the refix per-piece payment memory (ruling C: paid_pieces recording, sheet-side match/collision/ORPHAN handling, sealed-root refusal) and the supersede/heal clears — including the r-loop-10 zip-heal deferred clear and the .wedged daily-resume mechanism — behave as documented; (7) the '## Reject detail' section and the CSV columns describe the same population. Read the RULED design in item 7 of the accepted list first — the DESIGN is settled, DEFECTS IN IT are exactly what you are hunting.` },
  ...
  { key: 'tests-coverage', prompt:
    `Attack the TEST SUITE itself. The suite is the flip's ARMING GATE (tools/run_suite.sh, floor 619, currently 641 passing). Find fixes in ${LOOP_START}..HEAD whose behaviour survives deletion with the suite still green — apply the mutation in a scratch copy OUTSIDE the repo and run pytest to PROVE it, then restore. Also look for tests that assert a bug, tests that pass for the wrong reason (re-running SQL instead of the code, asserting source text, a fixture that makes the assertion vacuous), and orphaned threads or leaked state between tests. A fix with no real test is a real finding.` },
]
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-10 in-iteration fix set (commit 6dd2e64, 16 fixes on top of the r-loop-9 D1-D8 set 640651a..81d5f06). RULED SCOPE (Adnaan 2026-08-19): this is a FULL DEEP review of the whole codebase — review your lane's surface END TO END as if for the first time, not as a delta pass — AND, wherever your lane touches code changed by 6dd2e64, treat that commit as the prime suspect: it was written by the executor whose work this pass audits, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 11: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
