// r-loop-17 review workflow (FIX-IN-ITERATION pass per R8_IMPLEMENTATION_PLAN
// §4, RULED 2026-08-19: confirmed findings are fixed by the executor in this
// same iteration with full §2/§3 discipline, and iteration 18 — run ONLY if
// this one is not quiet — reviews those fixes; stop at the first quiet
// iteration). Edited from the committed iter16 snapshot: regressions lane
// retargeted at the ITERATION-16 J FIX SET (commits c4f1fda J1
// strict-encode session_id, c0d37de J2 bound-aware INP_OSKEYS trigger +
// J3/J4 tests-only pins, eaaee0d J5 fail-closed daily scan, ddc6da8 J6
// Comma named token + floor cba8fd2 — the executor's own, treat as prime
// target), accepted-behaviours 76-79 appended for the J fixes and Adnaan's
// 2026-08-19 J5/J6 rulings (mechanism defects stay reportable;
// 17/53/71/72/73/74 amended where the J fixes supersede or complete their
// mechanics), suite refreshed (782/floor 778). LOOP_START, 2-vote refute
// discipline and all 7 lanes kept.
export const meta = {
  name: 'flip-review-iter17',
  description: 'r-loop 17: fix-in-iteration adversarial review of the continuous-pipeline flip work, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-16 J fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 778 since the
   r-loop-16 J set (was 761 after the I set, 745 after the H set, 718
...
const LANES = [
  { key: 'regressions-r16', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-16 J fix set — commits c4f1fda (J1: safe_session_id measures the id's byte length with a STRICT utf-8 encode inside try/except UnicodeEncodeError -> folder-name fallback), c0d37de (J2: validate_session resolves the session binding into aux['bound_literals'] via the new _session_bound_literals — session raw/keybind.json anchored on expected_game, built-in fallback, KEYBIND_PATCHES, degrade-with-note — and map_reasons filters the engine's pattern-only os_keys by key_canonical against it: unbound keeps the blocking INP_OSKEYS, bound becomes an advisory; PLUS J3/J4 tests-only pins for the fix_sync remap credited-strip and the retrim naive guard), eaaee0d (J5, RULED: the day-agnostic resume scan routes through reports._report_day_dirs — None refuses the tick loudly, [] keeps the first-ever-send fresh path), ddc6da8 (J6, RULED: ',' -> 'Comma' in _KEY_DISPLAY + the 'comma' -> ',' literal alias; the checker's comma arm untouched) and the floor bump cba8fd2 (SUITE_FLOOR 778) — all written by the same executor whose work you are checking; this loop's worst findings have always been regressions from the previous iteration's fixes. Read each commit with git show IN FULL, then attack: (c4f1fda) the strict-encode try block's placement (does any accept arm still run on the unencodable id first?), surrogate-PAIR ids and other odd-but-encodable classes wrongly falling back, the boundary semantics vs I5; (c0d37de J2) the aux wiring against every validate_session caller (both drivers pass expected_game=row game — what about None?), _session_bound_literals vs fix_key_hygiene drift (game_name anchoring, patches, unusable-file fallback), key_canonical display-vs-canonical mismatches for every OS-pattern token the engine can emit (CapsLock, PrintScreen, F5, media keys), the degrade note's conservatism, the advisory's needle safety against validate._map_qa_issues and reports parsing, INP_OSKEYS params shape change consumers; (c0d37de J3/J4) do the pins pass for the wrong reason (a stub too strong, an assert satisfied vacuously); (eaaee0d) BOTH sides of the new guard (None refuses with NOTHING built/sent/stamped; [] proceeds), the regen-guard interplay (two listings, asymmetric failures both ways), the loud line's alert-dedup stability, any OTHER reports-dir consumer still listing inline; (ddc6da8) the 'Comma' round-trip through EVERY display consumer (checker fan-out lit2sem, hygiene, fix_v1_to_v2, fix_actions_context, fix_sync_from_v1 key_internal), the new literal alias against normalize_event_key (an EVENT key named 'comma'), invert_keybind/key_binding.json coverage for ',' binds on the v1 path, collision risk of the token 'Comma' with a real letter-key name. Report at most 5 findings — the ones you can PROVE.` },
  ...
  { key: 'tests-coverage', prompt:
    `Attack the TEST SUITE itself. The suite is the flip's ARMING GATE (tools/run_suite.sh, floor 778, currently 782 passing). Find fixes in ${LOOP_START}..HEAD whose behaviour survives deletion with the suite still green — apply the mutation in a scratch copy OUTSIDE the repo and run pytest to PROVE it, then restore. Also look for tests that assert a bug, tests that pass for the wrong reason (re-running SQL instead of the code, asserting source text, a fixture that makes the assertion vacuous), and orphaned threads or leaked state between tests. A fix with no real test is a real finding. A test COHORT that runs a pin only where the pinned behavior is a no-op (the r15 #6 class — every H2 test used kamla) is a real finding too.` },
]
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-16 J fix set (c4f1fda..cba8fd2: J1-J6 + floor 778, on top of the r-loop-15 I set bfd96b7..35a0433, the r-loop-14 H set 1dd69fa..37d7d88 and the r-loop-13 G set abf052b..a5fc1a0). This is review iteration 17 — a FIX-IN-ITERATION pass (RULED 2026-08-19): confirmed findings are fixed by the executor in this same iteration and iteration 18 — the LAST before the checkpoint — reviews those fixes; the stream stops at the first quiet iteration. A false confirm burns iteration budget on a non-bug; a false quiet ships a defect toward the flip — verify accordingly. Scope (plan §4, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the r-loop-16 J commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 17: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
