// r-loop-16 review workflow (FIX-IN-ITERATION pass per R8_IMPLEMENTATION_PLAN
// §4, RULED 2026-08-19: confirmed findings are fixed by the executor in this
// same iteration with full §2/§3 discipline, and iteration 17 — run ONLY if
// this one is not quiet — reviews those fixes; stop at the first quiet
// iteration). Edited from the committed iter15 snapshot: regressions lane
// retargeted at the ITERATION-15 I FIX SET (commits bfd96b7 I1 caseless-key
// exemption, 348c93d I2 combo-half writer strip, 843a2ec I3 slug-half OW
// pins, ee35d3f I4 v1 tz guard, 7169922 I5 session_id length bound, a93847d
// I6 composition-arm pins, f3f131e I7 H5 gone-is-gone coaching, fd3ea1f I8
// portable fix_sync copy + floor 35a0433 — the executor's own, treat as
// prime target), accepted-behaviours 70-75 appended for Adnaan's 2026-08-19
// I rulings (mechanism defects stay reportable; 17/62/65/66/67 amended where
// the I fixes supersede or complete their mechanics — 65's amendment is
// load-bearing: same-path-terminal is now THE RULED DESIGN), suite refreshed
// (765/floor 761). LOOP_START, 2-vote refute discipline and all 7 lanes
// kept.
export const meta = {
  name: 'flip-review-iter16',
  description: 'r-loop 16: fix-in-iteration adversarial review of the continuous-pipeline flip work, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-15 I fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 761 since the
   r-loop-15 I set (was 745 after the H set, 718 after the G set, 692
...
65. r-loop 14 H5 (r14 #5), AMENDED by the r-loop-15 RULING (Adnaan
   ...
   quarantined-path heal. This entry's previous phrasing ("a
...
71. r-loop 15 I1 (r15 #4, RULED bfd96b7): check_session_v2's key-token
   ...
   re-FAILed anywhere in the chain) are findings.
72. r-loop 15 I2 (r15 #5, RULED 348c93d): the delivery writers enforce
   ...
   diverging from rule satisfaction) are findings.
73. r-loop 15 I4 (r15 #7, ee35d3f): fix_v1_to_v2 repairs a NAIVE v1
   ...
   every host. Defects are findings.
74. r-loop 15 I5 (r15 #8, 7169922): safe_session_id's shared accept
   ...
   back) plus byte-vs-char semantics. Defects are findings.
...
const LANES = [
  { key: 'regressions-r15', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-15 I fix set — commits bfd96b7 (I1: check_session_v2's key-token case clause flags only tokens that HAVE case — t.lower() == t and t.upper() != t and not t.isdigit — so caseless symbol keys stay in deliveries), 348c93d (I2: resolve_actions grows a keyword-only credited_out set; _v2_rows takes a required rules param and keeps a token only if bound AND credited by a fully-satisfied rule, strip-and-counting the rest; fix_key_hygiene mirrors it at its resolve call; the fix_sync_from_v1 remap applies kset &= credited), 843a2ec (I3, tests-only: OW-ledger slug-half discriminator pins beside the H2 block), ee35d3f (I4: fix_v1_to_v2 repairs a naive created_at_utc to UTC before the astimezone write; the sweep added the same guard to tools/retrim_v2_session.py), 7169922 (I5: safe_session_id bounds the id to 200 utf-8 bytes), a93847d (I6, tests-only: _joint_edge_short tail-arm + chat-head pins), f3f131e (I7, RULED: the H5 vanished-arm detail and loud line gain the rename-re-upload coaching; the same-path dead end is the ruled design), fd3ea1f (I8, RULED: fix_sync_from_v1 copies delivery files with shutil.copy2, subprocess import removed, the H9c twin's _portable_cp stub removed) and the floor bump 35a0433 (SUITE_FLOOR 761) — all written by the same executor whose work you are checking; this loop's worst findings have always been regressions from the previous iteration's fixes. Read each commit with git show IN FULL, then attack: (bfd96b7) the case clause against unicode case pairs and mixed scripts (a token whose upper() differs unexpectedly, a cased non-latin letter the writer emits), the digits arm, the button set-membership untouched, the hygiene/key_display round-trip agreement claim, any consumer that still assumes INP_TOKEN_CASE fires for symbols; (348c93d) credited_out semantics vs dead_literals (context path: a ctx-blocked token credited by another rule; gating SKIPPED paths), the required rules param against every _v2_rows caller and any uncovered tool/test path, credit computed with motion (False,False) vs motion-bound keys, strip_stats double-counting, the hygiene mirror's resolve-then-strip ordering (can stripping change what re-resolution would produce NEXT attempt?), buttons named in combo groups, collapse_ambiguous_runs interplay (can a collapse empty a credited token's actions?), the fix_sync remap's kset &= credited change of unbound-token handling; (843a2ec + a93847d) do the new pins pass for the wrong reason (stubs too strong, asserts vacuous), and did any stub weaken an existing pin; (ee35d3f) the guard's placement before the head_cut_s adjustment, the retrim sweep guard's interaction with its callers, the TZ-forcing test restoring host state; (7169922) the 200-byte boundary against real derived-name lengths (split-manifest, -analysis, -pN chains), multibyte ids, fallback COLLISIONS (two bundles whose folder name collides after fallback), interplay with the H7 control-char arm; (f3f131e) the new detail/loud-line text against EVERY event-anchored query (prev_md5= rsplit parse, ZIP_ADJ_CHANGED prefix, split segment/verified at/staged prefixes) and the 300-char cap, the ruling pin's zero-event-churn assert vs anything scan legitimately writes on a restore listing, the corrected comment's truth; (fd3ea1f) shutil.copy2 vs cp -c semantics (metadata, existing-dest behavior), the unstubbed twin on Linux, anything else that still imported subprocess from that tool. Report at most 5 findings — the ones you can PROVE.` },
  { key: 'payment-split', prompt:
    `Attack the two-mark payment logic in pipeline/reports.py (build_sheet_rows, _tree_has_uncounted_accepted, _mem_reconcile_failures, _stamp, mark_uploads_reported, mark_accepted_reported, pending_daily_send, write_payment_sheet) plus its callers in pipeline/run.py (fresh + resume daily paths, the wedge machinery) and tools/recal_regen_sheets.py, tools/recal_refix_reset.py, tools/recal_rebuild_reset.py, pipeline/ledger.py (supersede, update_where_md5, paid_pieces) and pipeline/ingest.py (quarantine heal). Prove or disprove, by RUNNING code against a scratch sqlite ledger in /tmp: (1) uploaded hours are still counted exactly once across all sheets; (2) every DELIVERED node's hours reach exactly one sheet, and none is lost — including under duration_raw_s=NULL roots (the new third re-entry arm) and re-cut trees (the new reconcile-against-DELIVERED void); (3) reject labels appear exactly once; (4) no root can re-enter forever silently; (5) a kill between the sheet send and either stamp cannot double-pay, and a supersede/heal INSIDE the stamp window cannot strand the corrected re-upload (the new md5 compare-and-set); (6) the refix per-piece payment memory, the sealed-root refusal, the supersede/heal clears, the zip-heal deferred clear, the wedge machinery (F1 guard + F5 durable alert) and the regen interlock (F10) behave as documented; (7) the '## Reject detail' section and the CSV columns describe the same population. Include the r-loop-13 changes on your surface: G1's durable '' adjudication (supersede breadcrumb, ZIP_ADJ_CHANGED marker, _stamp's counted_at arm), G5's rebuild-reset refusal/preserve/record, and G6's pending_daily_send_detail kinds — now joined by the r-loop-14 changes: H1's PRE-BUILD counted_at capture (one string into the record and both stamp calls; the resume replays it) and H3's split-artifact discard in the rebuild-reset teardown. NO r-loop-15 change landed on this surface (the I set's payment effect is upstream: fewer wrongful rejects reach you) — still hunt it whole. Read the RULED design in items 7, 21, 22, 34-35, 39-40, 45 and 55-61 of the accepted list first — the DESIGN is settled, DEFECTS IN IT are exactly what you are hunting.` },
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-15 I fix set (bfd96b7..35a0433: I1-I8 + floor 761, on top of the r-loop-14 H set 1dd69fa..37d7d88, the r-loop-13 G set abf052b..a5fc1a0 and the r-loop-12 set 986368f..0ad8747). This is review iteration 16 — a FIX-IN-ITERATION pass (RULED 2026-08-19): confirmed findings are fixed by the executor in this same iteration and iteration 17 reviews those fixes; the stream stops at the first quiet iteration. A false confirm burns iteration budget on a non-bug; a false quiet ships a defect toward the flip — verify accordingly. Scope (plan §4, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the r-loop-15 I commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 16: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
