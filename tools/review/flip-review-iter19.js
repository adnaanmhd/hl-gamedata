// r-loop-19 review workflow (the FIRST of the two RULED extra passes —
// Adnaan 2026-08-20, "i want to run two more iterations", superseding the
// stop-at-first-quiet rule: iterations 19 and 20 BOTH run. This pass
// reviews the r-loop-18 L fix set, which landed UNREVIEWED; if it is NOT
// quiet, its confirmed findings are fixed in-iteration (the M set) and
// iteration 20 reviews them; if it IS quiet, iteration 20 runs as a
// CONFIRMATION pass). Edited from the committed iter18 snapshot:
// regressions lane retargeted at the ITERATION-18 L FIX SET (commits
// e197244 L1 fix_v1_to_v2 whole-function degrade, 21c983e L2 usable-
// sidecars plan gate, f57b3ff L3 tests-only live pin of K2's anchor +
// floor 74b4a17 — the executor's own, landed unreviewed, treat as prime
// target), accepted-behaviours 86-88 appended for the L fixes (mechanism
// defects stay reportable; 17/81/82 amended where the L fixes complete or
// close their mechanics), suite refreshed (802/floor 798). LOOP_START,
// 2-vote refute discipline and all 7 lanes kept.
export const meta = {
  name: 'flip-review-iter19',
  description: 'r-loop 19: first of two RULED extra passes — adversarial review of the flip work incl. the UNREVIEWED r18 L fix set, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-18 L fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 798 since the
   r-loop-18 L set (was 789 after the K set, 778 after the J set, 761
   after the I set, 745 after the H set, 718 after the G set, 692
   after r12, 670 after F11).
...
const LANES = [
  { key: 'regressions-r18', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-18 L fix set — commits e197244 (L1: fix_v1_to_v2 degrades EVERY read of the player-typed v1 payload instead of crashing — dx/dy through the new _parse_motion with has_motion judging PARSED values, a str()/float()-coerced created_at_utc/trim parse that OMITS an unusable stamp for fix_sessionjson_recompute to synthesize, non-dict canonical/trim normalized to {}, session.json read via _read_session_json), 21c983e (L2: has_raw_sidecars — the single shared plan gate both drivers route through — now requires raw/metadata.json to parse to a dict, so present-but-unusable sidecars read as no-sidecars and fall back to CSV-level fixes; typed FixFailed belt-and-braces at retranslate_from_sidecars' own read), f57b3ff (L3 tests-only: K2's game_name=slug anchor pinned live on an outer_wilds-ledger v1 conversion with degraded canonical metadata and a parsed-but-unusable keybind) and the floor bump 74b4a17 (SUITE_FLOOR 798) — all written by the same executor whose work you are checking, and ALL LANDED UNREVIEWED (iteration 18 was the last pass under the previous ruling — you are the FIRST review these commits get); this loop's worst findings have always been regressions from the previous iteration's fixes. Read each commit with git show IN FULL, then attack: (e197244) the has_motion SEMANTICS CHANGE against every consumer of the conversion's output — the old string test counted junk and '0.0' cells as motion, the new rule blanks all-junk AND all-zero columns: can a real capture whose motion is genuinely all-zero now ship the blank no-capture form where a checker reason, the mouse_capture data_quality flag, or the delivery contract distinguishes blank from zero? Then _parse_motion's float(v or 0) truthiness (v=0, '', None, False, '0', 'inf', 'nan', numeric-typed cells — which values short-circuit the or-0, which raise into the except arm, which pass the isfinite test), the omitted-stamp interplay (a WRONG-but-parseable stamp still ships adjusted verbatim while an unusable one is OMITTED and synthesized by fix_sessionjson_recompute — trace what recompute synthesizes FROM and whether omitted-vs-wrong changes any downstream surface), the stray-key_binding early-return arm now fed s={} by _read_session_json on a corrupt session.json (an already-v2 session with a corrupt session.json: which arm does it take now, and is the outcome worse than before?), the trim coercion (a junk head_cut_s now discards the WHOLE stamp — verify the committed pin's claim that a junk trim keeps the valid stamp), and the folder-name session_id fallback from a non-dict canonical against the safe_session_id joins; (21c983e) every has_raw_sidecars consumer for GATE DRIFT (pipeline/validate.py's own raw handling vs the plan gate — can validation's aux or reasons say sidecars-present while plan_fixes reads absent, and does anything ACT on the disagreement?), the QA_FAIL_UNMAPPED route under the new gate (with unusable metadata, what does plan_fixes now plan — a CSV-level fallback, or UNFIXABLE/terminal? prove the actual outcome and compare its harm against the old crash-both-attempts path), the per-plan reparse (has_raw_sidecars now json.loads metadata.json on every call — call frequency, large-file cost, and the gate-vs-read TOCTOU when the file changes between the gate and retranslate_from_sidecars' read; the typed FixFailed claims to cover the residual — verify its kind classification), and the errors='replace' tolerance (mojibake that still PARSES to a dict counts as usable — can replacement-char metadata poison the retranslate WORSE than the CSV fallback it superseded?); (f57b3ff) does the pin pass for the WRONG reason (is W/A/S survival supplied by KEYBIND_PATCHES alone rather than resolve_keybind's built-in fallback? would an exe_name-derived or otherwise subtly-wrong anchor still pass it?), and the cohort gaps that REMAIN (do L1's own new tests run the fixed lines only where they are no-ops — game, route, or arm-wise?); (74b4a17) the floor arithmetic (802 passed − 4 = 798). Report at most 5 findings — the ones you can PROVE.` },
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-18 L fix set (e197244..74b4a17: L1-L3 + floor 798, on top of the r-loop-17 K set c99309e..6f97449, the r-loop-16 J set c4f1fda..cba8fd2, the r-loop-15 I set bfd96b7..35a0433 and the r-loop-14 H set 1dd69fa..37d7d88; suite 802/802 on both hosts). This is review iteration 19 — the FIRST of the two RULED extra passes (Adnaan 2026-08-20: iterations 19 and 20 BOTH run, superseding the stop-at-first-quiet rule). The L set landed UNREVIEWED — iteration 18 was the last pass under the previous ruling — so this pass is the first review those commits get. If this pass is NOT quiet, its confirmed findings are fixed in-iteration (the M set) and iteration 20 reviews them; if it IS quiet, iteration 20 runs as a CONFIRMATION pass before the checkpoint. A false confirm burns an iteration on a non-bug; a false quiet ships a defect toward the flip — verify accordingly. Scope (plan §4, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the r-loop-18 L commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, they landed unreviewed, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 19: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
