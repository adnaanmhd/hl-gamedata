// r-loop-12 review workflow (WARRANTED by Adnaan 2026-08-19: iteration 11 was
// not quiet, so two more iterations run; 12 is fix-in-iteration per plan §5).
// Edited from the committed iter11 snapshot: regressions lane retargeted at
// the ITERATION-11 FIX SET F1-F11 (commits 82e42df..2f5ca04 + floor 09cbf20,
// the executor's own — treat as prime target), accepted-behaviours 34-44
// appended for the r11 rulings (mechanism defects stay reportable; 17/22/30/33
// amended where F1/F2/F5/F7 supersede their mechanics), suite refreshed
// (674/floor 670). LOOP_START, 2-vote refute discipline and the
// tests-coverage lane kept.
export const meta = {
  name: 'flip-review-iter12',
  description: 'r-loop 12: adversarial review of the continuous-pipeline flip work, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-11 fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 670 since the
   r-loop-11 F set (was 619 since D8, 578 before that).
...
40. r-loop 11 F7 (#7, payment-surface, to be surfaced to Adnaan): the
   ...
   WHERE tightened. Defects in the CAS are findings.
41. r-loop 11 F8 (#9): the DISCOVERED-media reclaim sweep and the
   ...
   while never-successful rows are unchanged.
...
const LANES = [
  { key: 'regressions-r11', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-11 fix set — commits 82e42df, b5cc713, e6a1ccd, 206815c, 88c7a4f, cd178f3, 71c1087, 4ad510e, a8a1996, 5696205, 2f5ca04 (F1-F11) plus the floor bump 09cbf20, all written by the same executor whose work you are checking — this loop's worst findings have always been regressions from the previous iteration's fixes; do not extend it any benefit of the doubt. Read each commit with git show IN FULL, then attack: (F1, run.py) the fresh-path guard for a wedged/pending TODAY — attack the settled-quiet classification (sent + unreadable record refuses SILENTLY — can a genuinely pending day be silently parked forever?), the guard vs the scan's own settled test drifting apart, and ordering against the day-agnostic resume; (F2, reports.py) the reconcile-against-DELIVERED void — attack false voids locking out genuinely-new hours (a memory row with seconds NULL, a legitimately re-delivered identical piece beside one dropped piece), whether REJECT LABELS still flow on a void tree, the per-sheet loud-line loop, and _tree_has_uncounted_accepted's parity with build_sheet_rows; (F3, fix.py) the host-tuple re-raise in fix_lagshift_csv; (F4, fix.py) hygiene resolving work/raw/keybind.json — attack the REROUTED-game interplay (review-2 #5 says on reroute the built-in for the corrected game governs — hygiene now honors the sidecar keybind even when the ledger game was rerouted; is a plan with FIX_REROUTE_GAME + hygiene reachable without a retranslate in between?), game_name=slug round-tripping through game_key_from_name, and an unusable sidecar keybind falling back correctly; (F5, run.py) the durable wedge alert — attack alert spam when the .wedged write keeps failing, the legacy plain-text upgrade path, the tmp-file rewrite, and whether _attempt_wedge_alert's file re-read races the scan; (F6, validate.py/run.py/continuous.py/reports.py) the duration backfill + third re-entry arm — attack double-pay paths (backfilled root re-entering late AFTER the third arm paid its accepted side; conservation across sheets), the backfill racing supersede (stale probed duration written onto NEW bytes), and the arm's interplay with paid-piece memory/void; (F7, ledger.py/reports.py/run.py) the compare-and-set stamps — attack sids with NULL md5 in the snapshot (children!), the regen tool NOT passing md5s (blind stamps remain there — reachable harm?), _stamp's return-count semantics feeding the printed totals, and the resume pre-filter + CAS interaction; (F8, run.py/continuous.py) the stint-scoped anchors — attack generations whose gen-1 never left DISCOVERED/DOWNLOADING (supersede from DISCOVERED — the anchor still lands on gen-1; harm?), and the outside-event set correctness; (F9, translator) safe_session_id — attack bypasses (os.altsep, NUL bytes, unicode normalization, a '..'-free absolute path?) and the assert under python -O; (F10, reports.py) the extended interlock — attack regen-record naming/day ordering; (F11) the five new pins for vacuous assertions. Also attack the ~30 new/changed tests across the set for asserting the wrong thing. Report at most 5 findings — the ones you can PROVE.` },
  { key: 'payment-split', prompt:
    `Attack the two-mark payment logic in pipeline/reports.py (build_sheet_rows, _tree_has_uncounted_accepted, _mem_reconcile_failures, _stamp, mark_uploads_reported, mark_accepted_reported, pending_daily_send, write_payment_sheet) plus its callers in pipeline/run.py (fresh + resume daily paths, the wedge machinery) and tools/recal_regen_sheets.py, tools/recal_refix_reset.py, tools/recal_rebuild_reset.py, pipeline/ledger.py (supersede, update_where_md5, paid_pieces) and pipeline/ingest.py (quarantine heal). Prove or disprove, by RUNNING code against a scratch sqlite ledger in /tmp: (1) uploaded hours are still counted exactly once across all sheets; (2) every DELIVERED node's hours reach exactly one sheet, and none is lost — including under duration_raw_s=NULL roots (the new third re-entry arm) and re-cut trees (the new reconcile-against-DELIVERED void); (3) reject labels appear exactly once; (4) no root can re-enter forever silently; (5) a kill between the sheet send and either stamp cannot double-pay, and a supersede/heal INSIDE the stamp window cannot strand the corrected re-upload (the new md5 compare-and-set); (6) the refix per-piece payment memory, the sealed-root refusal, the supersede/heal clears, the zip-heal deferred clear, the wedge machinery (F1 guard + F5 durable alert) and the regen interlock (F10) behave as documented; (7) the '## Reject detail' section and the CSV columns describe the same population. Read the RULED design in items 7, 21, 22, 34-35 and 39-40 of the accepted list first — the DESIGN is settled, DEFECTS IN IT are exactly what you are hunting.` },
  { key: 'driver-core', prompt:
    `Review pipeline/continuous.py end to end: lanes, ownership/claims, cooldowns, the media cap, autoscale, drain/shutdown, digest, housekeeping cadences. Look for races, lost work, states that can never be left, and counters that can mislead an operator at the flip. Include the r-loop-11 changes on your surface: the validate-time duration backfill in _validate_one, and the stint-scoped disc_media anchor in _stuck_lines.` },
  { key: 'fix-validate', prompt:
    `Review pipeline/fix.py, pipeline/validate.py and pipeline/gate.py end to end: plan ordering, the fix budget (FIX_RETRIES=2), reason mapping and bins, what is fixable vs unfixable, and every path that can reject a session that should have been delivered or deliver one that should not. Pay particular attention to the QA-string mapping table: a needle that matches the wrong FAIL, a FAIL with no needle, or a mapping whose planned fix cannot clear the FAIL it was mapped from. Include the r-loop-11 changes on your surface: fix_key_hygiene's session-keybind resolution (F4), fix_lagshift_csv's host re-raise (F3), and the probed_duration_s threading through _metrics (F6).` },
  ...
  { key: 'ops-tools', prompt:
    `Review tools/vm_setup.sh, tools/run_suite.sh, tools/recal_regen_sheets.py, tools/recal_verify_tree.py, tools/recal_rebuild_reset.py, tools/recal_refix_reset.py, pipeline/systemd/* and FLIP_RUNBOOK.md. Look for anything that can leave production unarmed, delete live data, block the payment endgame, or give an operator a false diagnosis. Include the r-loop-11 changes on your surface: the extended pending_daily_send interlock (F10 — regen record without its done marker) and the wedge machinery both reset tools now sit behind.` },
  { key: 'tests-coverage', prompt:
    `Attack the TEST SUITE itself. The suite is the flip's ARMING GATE (tools/run_suite.sh, floor 670, currently 674 passing). Find fixes in ${LOOP_START}..HEAD whose behaviour survives deletion with the suite still green — apply the mutation in a scratch copy OUTSIDE the repo and run pytest to PROVE it, then restore. Also look for tests that assert a bug, tests that pass for the wrong reason (re-running SQL instead of the code, asserting source text, a fixture that makes the assertion vacuous), and orphaned threads or leaked state between tests. A fix with no real test is a real finding.` },
]
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-11 F1-F11 fix set (commits 82e42df..2f5ca04 + floor 09cbf20, on top of the r-loop-10 set 6dd2e64 and the r-loop-9 set 640651a..81d5f06). Scope (plan §5, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the F1-F11 commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 12: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
