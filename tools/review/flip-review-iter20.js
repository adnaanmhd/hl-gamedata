// r-loop-20 review workflow (RULED Adnaan 2026-08-20, mid-session message
// superseding both the stop-after-20 amendment and the per-launch headroom
// ask: iteration 20 runs automatically; if NOT quiet, its confirmed
// findings are fixed in-iteration (the N set) and iteration 21 launches
// automatically; if 20 or 21 IS quiet, the independent e2e launches
// automatically; iteration 22's results are the checkpoint. This pass
// reviews the r-loop-19 M fix set, which landed UNREVIEWED). Edited from
// the committed iter19 snapshot: regressions lane retargeted at the
// ITERATION-19 M FIX SET (commits a6af11d M2 gate requires parseable
// started_at, cb8cfd4 M1 head-offset contract never fabricated, cb42dae
// M3 ragged-row padding, e9f913b M4 ''-supersede stamp preservation
// [payment-surface, RULED], 6129872 M5 truthful aux['has_raw'], 78e384e
// M6+M7 recompute overflow degrade + STR_SJ_INVALID mapping, 1a023fb M8
// tests-only pins + floor a239fad — the executor's own, landed
// unreviewed, treat as prime target), accepted-behaviours 89-95 appended
// for the M fixes (mechanism defects stay reportable; 17/55/86/87 amended
// where the M fixes complete or close their mechanics), suite refreshed
// (821/floor 817). LOOP_START, 2-vote refute discipline and all 7 lanes
// kept.
export const meta = {
  name: 'flip-review-iter20',
  description: 'r-loop 20: adversarial review of the flip work incl. the UNREVIEWED r19 M fix set, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-19 M fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 817 since the
   r-loop-19 M set (was 798 after the L set, 789 after the K set, 778
   after the J set, 761 after the I set, 745 after the H set, 718
   after the G set, 692 after r12, 670 after F11).
...
90. r-loop 19 M1 (r19 #1 BLOCKER + #4≡#6 + #10, cb8cfd4):
   ...
   still crashes the route) are findings. Amends entry 86.
...
92. r-loop 19 M4 (r19 #5, payment-surface, RULED fix-now by Adnaan
   ...
   confuses the sheet math) are findings. Amends entry 55.
...
const LANES = [
  { key: 'regressions-r19', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-19 M fix set — commits a6af11d (M2: has_raw_sidecars — the single shared plan gate both drivers route through — additionally requires a parseable recording.started_at_utc, judged by the module-level _utc SHARED with retranslate_from_sidecars; retranslate's non-dict-recording untyped AttributeError sibling fixed to the typed 'cannot derive the head offset' FixFailed; two 'usable' test controls re-modeled to genuinely usable sidecars), cb8cfd4 (M1, the r19 BLOCKER's fix: fix_v1_to_v2's stamp/trim resolution runs BEFORE any write and never fabricates the created−started head-offset contract — sidecar-usable route: unusable stamp + usable head cut RECOVERS created = started_at_utc + head_cut with a fixlog note, unusable head cut refuses with a typed FixFailed naming canonical.trim; no-sidecar route: junk trim VALUE/shape keeps a parseable stamp at head 0.0, unusable stamp keeps omit-and-synthesize; OverflowError joins every parse/arithmetic guard; new probe _v1_sidecar_started applies the M2 usability rule at work root AND raw/, each file located INDEPENDENTLY), cb42dae (M3: a short v1 body row pads to the header width with '' cells right after the header check; the other eight _read_csv consumers verdicted shielded in the commit), e9f913b (M4, payment-surface, RULED: ledger.supersede with the '' unknowable-md5 sentinel over a REAL stored md5 PRESERVES uploaded/accepted stamps + duration_raw_s + tree seal while every other reset stays; the download-time deferral owns byte adjudication), 6129872 (M5, RULED: validate's aux['has_raw'] routes through fix.has_raw_sidecars so the stored fixable field and the reject labels are truthful; raw_by_sid stays existence-based), 78e384e (M6+M7: fix_sessionjson_recompute degrades on OverflowError incl. the negative-offset astimezone sibling; 'session.json unreadable' + 'session.json is not a JSON object' map to STR_SJ_INVALID, fixable), 1a023fb (M8 tests-only: failing-side pins for L1's non-dict-canonical arm, L1's str() coercion, and L2's errors='replace' read) and the floor bump a239fad (SUITE_FLOOR 817) — all written by the same executor whose work you are checking, and ALL LANDED UNREVIEWED (you are the FIRST review these commits get); this loop's worst findings have always been regressions from the previous iteration's fixes. Read each commit with git show IN FULL, then attack: (cb8cfd4) the RECOVERY arithmetic — created = started_at_utc + head_cut: tz/naive handling of started_at, head-cut edge values that pass the guards yet shift the stamp (negative cuts, sub-second precision, float rounding), and whether the fixlog note matches what is written; the refusal path — does the typed FixFailed really leave the work dir byte-identical (probe attempt-2 re-entrancy after a refusal), what kind does apply_fixes classify it as, and what terminal label does the player see; the crash-between-moves split — root and raw/ can hold DIFFERENT metadata.json contents: which one wins _v1_sidecar_started and is that defensible; the no-sidecar junk-trim arm keeps a stamp WRITTEN FOR THE UNTRIMMED recording at head 0.0 — trace whether recompute's duration compare, qa-v2's duration check, or the delivered created_at is misled where the v1 payload SAID there was a trim; (a6af11d) a WRONG-but-parseable started_at (gate answers True, retranslate derives a wrong head offset — pre-existing or worsened?), naive-vs-aware stamp handling agreement between gate and consumer, and whether re-modeling the two 'usable' controls WEAKENED what they originally pinned; (cb42dae) the pad semantics — '' cells flow into _parse_motion (0.0) and key splits: can a padded row flip the has_motion accounting or the blank-vs-zero form entry 86 ships, and do longer-than-header rows really degrade everywhere the commit claims; (e9f913b) BOTH money directions against a scratch sqlite ledger — a genuinely-new-bytes zip re-upload whose stamps are preserved and which NEVER downloads (are the stranded stamps money-safe in every row state — REJECTED with counted labels, DELIVERED with accepted stamps?), and the preserve→download→ZIP_ADJ_CHANGED→clear chain end-to-end (incl. the tree-seal and duration_raw_s halves, and whether any preserved-but-reset field combination confuses build_sheet_rows, _tree_has_uncounted_accepted, or the reconcile void); (6129872) the lazy-import cycle claim (prove fix.py holds no module-level validate import), re-grep every aux['has_raw'] consumer yourself, and the CHANGED stored-fixable semantics against every reader of the ruled reject surface (session_reject_labels, reports, the dossier); (78e384e) both new needles against EVERY earlier needle in the mapping table (order matters — prove no earlier needle matches the two strings first), the entry-11 invariant (STR_SJ_INVALID must NEVER route to FIX_RETRANSLATE — trace the plan for an unreadable session.json WITH usable sidecars), and the M1 interplay (a sidecar-usable session whose recompute overflows — does the synthesize-from-now stamp leak into the head-offset contract M1 just sealed?); (1a023fb) do the three pins pass for the RIGHT reason (mutate scratch copies yourself), and the cohort gaps that REMAIN in test_r_loop19.py (is any NEW M arm pinned only where it is a no-op?); (a239fad) the floor arithmetic (821 passed − 4 = 817). Report at most 5 findings — the ones you can PROVE.` },
  { key: 'payment-split', prompt:
    `Attack the two-mark payment logic in pipeline/reports.py (build_sheet_rows, _tree_has_uncounted_accepted, _mem_reconcile_failures, _stamp, mark_uploads_reported, mark_accepted_reported, pending_daily_send, write_payment_sheet) plus its callers in pipeline/run.py (fresh + resume daily paths, the wedge machinery) and tools/recal_regen_sheets.py, tools/recal_refix_reset.py, tools/recal_rebuild_reset.py, pipeline/ledger.py (supersede, update_where_md5, paid_pieces) and pipeline/ingest.py (quarantine heal). Prove or disprove, by RUNNING code against a scratch sqlite ledger in /tmp: (1) uploaded hours are still counted exactly once across all sheets; (2) every DELIVERED node's hours reach exactly one sheet, and none is lost — including under duration_raw_s=NULL roots (the new third re-entry arm) and re-cut trees (the new reconcile-against-DELIVERED void); (3) reject labels appear exactly once; (4) no root can re-enter forever silently; (5) a kill between the sheet send and either stamp cannot double-pay, and a supersede/heal INSIDE the stamp window cannot strand the corrected re-upload (the new md5 compare-and-set); (6) the refix per-piece payment memory, the sealed-root refusal, the supersede/heal clears, the zip-heal deferred clear, the wedge machinery (F1 guard + F5 durable alert) and the regen interlock (F10) behave as documented; (7) the '## Reject detail' section and the CSV columns describe the same population. Include the r-loop-13 changes on your surface: G1's durable '' adjudication (supersede breadcrumb, ZIP_ADJ_CHANGED marker, _stamp's counted_at arm), G5's rebuild-reset refusal/preserve/record, and G6's pending_daily_send_detail kinds — now joined by the r-loop-14 changes: H1's PRE-BUILD counted_at capture (one string into the record and both stamp calls; the resume replays it) and H3's split-artifact discard in the rebuild-reset teardown — and by the r-loop-16 change ON THIS SURFACE: J5's fail-CLOSED day-agnostic resume scan (RULED, entry 78 — run.py's send_daily_report_if_due now routes its listing through reports._report_day_dirs and refuses the tick on None; verify both sides of the guard, the regen-guard interplay under asymmetric listing failures, and that nothing is built/sent/stamped on a refused tick) — and by the r-loop-19 change ON THIS SURFACE, LANDED UNREVIEWED (prime suspect): M4's stamp-preserving '' supersede (entry 92, RULED — ledger.supersede with a falsy new_md5 over a REAL stored md5 preserves uploaded/accepted stamps + duration_raw_s + tree seal while state/attempts/bin/reasons/delivered fields/dossier/breadcrumb still reset; the download-time deferral owns the clear via the prev_md5 breadcrumb + durable ZIP_ADJ_CHANGED marker; verify BOTH money directions against a scratch ledger: no double-pay through a preserved-then-cleared stamp, no stranded hours when genuinely-new bytes arrive over a preserved stamp, the never-downloads case, and the interplay with entry 55's durable adjudication arms). Read the RULED design in items 7, 21, 22, 34-35, 39-40, 45, 55-61, 78 and 92 of the accepted list first — the DESIGN is settled, DEFECTS IN IT are exactly what you are hunting.` },
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-19 M fix set (a6af11d..a239fad: M1-M8 + floor 817, on top of the r-loop-18 L set e197244..74b4a17, the r-loop-17 K set c99309e..6f97449, the r-loop-16 J set c4f1fda..cba8fd2 and the r-loop-15 I set bfd96b7..35a0433; suite 821/821 on both hosts). This is review iteration 20 (RULED Adnaan 2026-08-20: if this pass is NOT quiet, its confirmed findings are fixed in-iteration — the N set — and iteration 21 reviews them; if it IS quiet, the independent e2e launches next). The M set landed UNREVIEWED — this pass is the first review those commits get. A false confirm burns an iteration on a non-bug; a false quiet sends a defect toward the e2e and the flip — verify accordingly. Scope (plan §4, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the r-loop-19 M commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, they landed unreviewed, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 20: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
