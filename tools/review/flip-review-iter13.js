// r-loop-13 review workflow (RE-RULED by Adnaan 2026-08-19: runs REGARDLESS
// of iteration 12's verdict, as a confirmation pass; if not quiet -> STOP and
// hand Adnaan the list). Edited from the iter12 script: regressions lane
// retargeted at the ITERATION-12 FIX SET (commits 986368f, cd8546a, 362ced9,
// 02bdeac, af35f44, 9270d50, 1f981c1, 75db7fd, b1bea89, 0ad8747 + floor
// 6b87023 — the executor's own, treat as prime target), accepted-behaviours
// 45-54 appended for the r12 rulings (mechanism defects stay reportable;
// 17/40/41/43 amended where the r12 fixes supersede their mechanics), suite
// refreshed (696/floor 692). LOOP_START, 2-vote refute discipline and the
// tests-coverage lane kept.
export const meta = {
  name: 'flip-review-iter13',
  description: 'r-loop 13: adversarial review of the continuous-pipeline flip work, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-12 fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 692 since the
   r-loop-12 fix set (was 670 after F11, 619 since D8).
...
45. r-loop 12 #1/#2: '' (the zip class's UNKNOWABLE-md5 sentinel, r10
   ...
   rule. Defects in this composition are findings.
46. r-loop 12 #3/#12: recal_rebuild_reset acquires and HOLDS the run
   ...
   stale-lock reclaim that comes along.
...
50. r-loop 12 #7: CNT_NOTIF_EDGE and CNT_CHAT_PII edge arms emit
   ...
   probed duration — mirroring the CNT_EDGE_NONGAMEPLAY arm.
...
const LANES = [
  { key: 'regressions-r12', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-12 fix set — commits 986368f ('' md5 sentinel in the stamps/resume), cd8546a (rebuild-reset run lock), 362ced9 (reclaim-marker grace re-arm), 02bdeac (fix_actions_context session keybind), af35f44 (structural-first cut-less plans), 9270d50 (notif/chat edge map-time CNT_SHORT), 1f981c1 (VLM sweep clamp), 75db7fd (OverflowError arms), b1bea89 (daily-send regen interlock + fail-closed interlocks), 0ad8747 (F6/F9 pins) and the floor bump 6b87023 — all written by the same executor whose work you are checking; this loop's worst findings have always been regressions from the previous iteration's fixes. Read each commit with git show IN FULL, then attack: (986368f) the ''-sentinel composition — can the not-in-snapshot unconditional arm double-pay a REAL supersede? does the NULL-duration discriminator false-skip a legitimately-NULL-duration recorded-'' sid (REJECTED roots on the accepted side)? does the resume real-vs-real rule strand hours the old rule protected?; (cd8546a) lock reclaim stealing a live driver's lock, release on every exit path; (362ced9) the marker event polluting other event-anchored queries (the F8 stint anchors, the worker-death count, _stuck_lines' other stints), same-second ts ties; (02bdeac) reroute interplay (review-2 #5) and unusable-sidecar fallback at the context site; (af35f44) plan orders for every reason combination — did structural-first break the gate/retrim orderings or double-emit steps?; (9270d50) the t±1.0 geometry vs plan_fixes' actual cuts, probed-vs-claimed edge disagreement; (1f981c1) probe failure paths (duration 0), the 24h cap interacting with legitimate long sessions; (75db7fd) the typed-FAIL text reaching map_reasons needles; (b1bea89) the UNKNOWN sentinel flowing into tool JSON/prints, the daily-send regen check racing the regen itself, FileNotFoundError-vs-ENOTDIR; (0ad8747 and the ~28 new tests in the set) vacuous assertions. Report at most 5 findings — the ones you can PROVE.` },
  ...
  { key: 'ops-tools', prompt:
    `Review tools/vm_setup.sh, tools/run_suite.sh, tools/recal_regen_sheets.py, tools/recal_verify_tree.py, tools/recal_rebuild_reset.py, tools/recal_refix_reset.py, pipeline/systemd/* and FLIP_RUNBOOK.md. Look for anything that can leave production unarmed, delete live data, block the payment endgame, or give an operator a false diagnosis. Include the r-loop-11/12 changes on your surface: the extended, fail-CLOSED pending interlocks (F10 + r12 #11/#13, now consulted by the daily send too), rebuild-reset's new held run lock (r12 #3/#12), and the wedge machinery both reset tools sit behind.` },
  { key: 'tests-coverage', prompt:
    `Attack the TEST SUITE itself. The suite is the flip's ARMING GATE (tools/run_suite.sh, floor 692, currently 696 passing). Find fixes in ${LOOP_START}..HEAD whose behaviour survives deletion with the suite still green — apply the mutation in a scratch copy OUTSIDE the repo and run pytest to PROVE it, then restore. Also look for tests that assert a bug, tests that pass for the wrong reason (re-running SQL instead of the code, asserting source text, a fixture that makes the assertion vacuous), and orphaned threads or leaked state between tests. A fix with no real test is a real finding.` },
]
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-12 fix set (986368f..0ad8747 + floor 6b87023, on top of the r-loop-11 F set 82e42df..2f5ca04 and the r-loop-10 set 6dd2e64). This is the CONFIRMATION pass Adnaan ordered to run regardless of iteration 12's verdict. Scope (plan §5, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the r-loop-12 commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 13: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
