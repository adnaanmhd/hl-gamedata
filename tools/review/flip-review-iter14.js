// r-loop-14 review workflow (fix-in-iteration per R8_IMPLEMENTATION_PLAN §4;
// quiet is judged AFTER fixing). Edited from the iter13 script: regressions
// lane retargeted at the ITERATION-13 G FIX SET (commits abf052b G1, 2d1b071
// G2, b383f05 G3, 161232f G4, dc97f9d G5, 792ef61 G6, 623ab8a G7, 231d4db G8,
// 82f5019 G9 + floor a5fc1a0 — the executor's own, treat as prime target),
// accepted-behaviours 55-60 appended for the G rulings (mechanism defects stay
// reportable; 17/45/46/50 amended where the G fixes supersede their
// mechanics), suite refreshed (722/floor 718). LOOP_START, 2-vote refute
// discipline and the tests-coverage lane kept.
export const meta = {
  name: 'flip-review-iter14',
  description: 'r-loop 14: adversarial review of the continuous-pipeline flip work, 2-vote refute',
  phases: [
    { title: 'Find', detail: 'lane finders: whole-codebase + delta + regressions from the r-loop-13 G fixes' },
...
17. The suite is CONT_DAILY_REPORTS-independent via a conftest autouse
   ...
   False-monkeypatch test (C9). SUITE_FLOOR default is 718 since the
   r-loop-13 G set (was 692 after r12, 670 after F11).
...
55. r-loop 13 G1 (#1/#2/#3): the zip-class '' adjudication is DURABLE.
   ...
   site. Defects in this composition are findings.
...
60. r-loop 13 G7 (#7): tools/fix_actions_from_v2 AND
   ...
   to the bundle dir name.
`

const LANES = [
  { key: 'regressions-r13', prompt:
    `Hunt for bugs INTRODUCED by the ITERATION-13 G fix set — commits abf052b (G1: zip-class '' adjudication made durable — supersede prev_md5 breadcrumb, ZIP_ADJ_CHANGED marker event at the download deferral, _stamp arm-2 re-keyed onto the marker vs counted_at, counted_at threaded through both mark functions and both daily paths), 2d1b071 (G2: FIX_RETRANSLATE keybind override only on rerouted plans via params), b383f05 (G3: notif/chat edge-vs-mid judged on dur_true), 161232f (G4: OverflowError arms — _check_session_json tuple, ts-stats block typed FAIL, video-duration compare, _verify_against_raw, analyze_sample _num), dc97f9d (G5: rebuild-reset payment refusal + --allow-reported preserve/record, tree-root-keyed paid pieces), 792ef61 (G6: pending_daily_send_detail kinds + shared guidance in both reset tools), 623ab8a (G7: safe_session_id + containment in fix_actions_from_v2 AND fix_sync_from_v1), 231d4db (G8: operator-twin keybind + lock pins), 82f5019 (G9: de-vacuoused r12 #7 edge tests) and the floor bump a5fc1a0 — all written by the same executor whose work you are checking; this loop's worst findings have always been regressions from the previous iteration's fixes. Read each commit with git show IN FULL, then attack: (abf052b) the marker LIKE query and the ts >= counted_at string compare (timestamp dialects, same-second ties, markers from PREVIOUS adjudication cycles landing at the same second), the breadcrumb rsplit parse against adversarial md5 content, arm-4 stamps whose self-heal never fires because the download never runs (terminal states, DUPLICATE verdicts), the kill-replay double-marker path, counted_at=None callers (recal_regen_sheets) still stamping unconditionally over an adjudicated slot, the marker perturbing ANY event-anchored query the commit message enumerates; (2d1b071) plans whose params lack 'rerouted' (older shapes), reroute plans where the driver could NOT resolve the corrected game, the C.GAMES belt-and-suspenders; (b383f05) head/tail asymmetry vs plan_fixes' actual t±1.0 cuts, dur_true=0 fallback when both probe and claim are 0; (161232f) the new FAIL strings against the _QA_STR_MAP needles (a wrong needle match, a needle collision), the wrapped ts-stats block skipping LEGITIMATE later checks after an early overflow raise mid-block, the per-event skip in _verify_against_raw hiding real mismatches; (dc97f9d) the parent-chain walk on cyclic/self-parented rows, paid pieces recorded under the wrong tree root, KEEP-state trees' DELIVERED children, the abort racing the dry-run, the preserved uploaded stamp against build_sheet_rows' three re-entry arms, the r6 test re-key; (792ef61) kind precedence when one day carries several states (wedged + regen), the wrapper's truthiness vs every consumer, the lock-test spy re-target; (623ab8a) safe_session_id fallback COLLISIONS (two traversal sessions collapsing onto one output dir), the assert crashing the tool instead of a refusal; (231d4db/82f5019 and the ~26 new tests in the set) vacuous assertions, stubs that make the pinned path unreachable. Report at most 5 findings — the ones you can PROVE.` },
  { key: 'payment-split', prompt:
    `Attack the two-mark payment logic in pipeline/reports.py (build_sheet_rows, _tree_has_uncounted_accepted, _mem_reconcile_failures, _stamp, mark_uploads_reported, mark_accepted_reported, pending_daily_send, write_payment_sheet) plus its callers in pipeline/run.py (fresh + resume daily paths, the wedge machinery) and tools/recal_regen_sheets.py, tools/recal_refix_reset.py, tools/recal_rebuild_reset.py, pipeline/ledger.py (supersede, update_where_md5, paid_pieces) and pipeline/ingest.py (quarantine heal). Prove or disprove, by RUNNING code against a scratch sqlite ledger in /tmp: (1) uploaded hours are still counted exactly once across all sheets; (2) every DELIVERED node's hours reach exactly one sheet, and none is lost — including under duration_raw_s=NULL roots (the new third re-entry arm) and re-cut trees (the new reconcile-against-DELIVERED void); (3) reject labels appear exactly once; (4) no root can re-enter forever silently; (5) a kill between the sheet send and either stamp cannot double-pay, and a supersede/heal INSIDE the stamp window cannot strand the corrected re-upload (the new md5 compare-and-set); (6) the refix per-piece payment memory, the sealed-root refusal, the supersede/heal clears, the zip-heal deferred clear, the wedge machinery (F1 guard + F5 durable alert) and the regen interlock (F10) behave as documented; (7) the '## Reject detail' section and the CSV columns describe the same population. Include the r-loop-13 changes on your surface: G1's durable '' adjudication (supersede breadcrumb, ZIP_ADJ_CHANGED marker, _stamp's counted_at arm), G5's rebuild-reset refusal/preserve/record, and G6's pending_daily_send_detail kinds. Read the RULED design in items 7, 21, 22, 34-35, 39-40, 45 and 55-60 of the accepted list first — the DESIGN is settled, DEFECTS IN IT are exactly what you are hunting.` },
...
const results = await pipeline(
  LANES,
  lane => agent(
    `You are reviewing a production data pipeline at ${REPO}. Branch main, HEAD carries the r-loop-13 G fix set (abf052b..a5fc1a0: G1-G9 + floor 718, on top of the r-loop-12 set 986368f..0ad8747 and the r-loop-11 F set 82e42df..2f5ca04). This is review iteration 14 (fix-in-iteration per the plan; every confirmed finding will be fixed before quiet is judged). Scope (plan §5, standing): review your lane's surface END TO END — whole-codebase, not just a delta pass — AND, wherever your lane touches code changed by the r-loop-13 G commits, treat those commits as the prime suspect: they were written by the executor whose work this pass audits, and this loop's worst findings have always been regressions from the previous iteration's fixes.
...
log(`r-loop 14: ${all.length} raised, ${confirmed.length} confirmed (${blockers.length} blockers), ${killed.length} refuted`)
